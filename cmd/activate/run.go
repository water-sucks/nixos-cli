...
func activateMain(cmd *cobra.Command, opts *cmdOpts.ActivateOpts) error {
	...
	if restartSystemd {
		// TODO: figure out if a proper dbus reply is received or not
		err = systemd.ReexecuteContext(ctx)
		if err != nil {
			log.Error("failed to re-execute the new systemd")
		}
	}
	...
	if err != nil {
		log.Errorf("failed to restart %s: %s", err)
		exitCode = 4
	}
	...
	if len(unitLists.Reload) > 0 {
		for unit := range unitLists.Reload {
			...
			if err != nil {
				log.Errorf("failed to parse unit file %s: %s", err)
				continue
			}
